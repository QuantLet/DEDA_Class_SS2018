...
# Extracting links
html_page = requests.get("https://www.tccb.gov.tr/receptayyiperdogan/konusmalar/")
links_soup = BeautifulSoup(html_page.text, 'html.parser')
#print(soup.find('div', {"id" : "divContentList"}).text)
links = []

for link in links_soup.findAll('a', attrs={'href': re.compile("/konusmalar")}):
    links.append("https://www.tccb.gov.tr/receptayyiperdogan/konusmalar/" + link.get('href'))
...
###NOTE###
#The links in the links list are chronologically ordered. Change the index of the list to assign the speech you'd like to analyse
...
text_file = open(date_stamp + " Output.txt", "w")
...
plt.savefig(title + " FrequentWords.png", transparent=True)
...
plt.title(title_en + ", Most Frequent Words")  
